...
@dp.message_handler(commands="set", chat_type=[types.ChatType.GROUP, types.ChatType.SUPERGROUP])
...
@dp.message_handler(commands='unset', chat_type=[types.ChatType.GROUP, types.ChatType.SUPERGROUP])
async def bot_unset(msg: types.Message):
    DBS.group_unset_channel(DBS, msg.chat.id)
    await msg.answer("kana oshirildi")

@dp.message_handler(commands='channel', chat_type=[types.ChatType.GROUP, types.ChatType.SUPERGROUP])
